...
The **student-PII masker** runs before every gateway invocation. Its always-on deterministic pass replaces **structured** record-linkable identifiers — SSN, student/case IDs, dates of birth, addresses, phone/email, card/lunch numbers — with **stable pseudonyms before any content enters a prompt or an audit record**. Free-text personal **names** (student or guardian) and other unstructured PII have no reliable regex signature and are masked only by the ML NER pass (`MASK_ENGINE=ml`, Comprehend/Presidio); that pass is **mandatory and fail-closed when real education-record text is processed** (`ALLOW_REAL_DATA=1` → the masker raises rather than degrade to regex-only), and best-effort in demo/fixture mode. Stable pseudonyms allow cross-call tracing for audit without exposing FERPA-protected identifiers or COPPA-protected data for under-13 learners. This is the EDU analog of HIPAA Safe-Harbor masking, tuned to FERPA identifiers and COPPA's heightened bar.
