...
| **8** | No unified edge + regional one-shot deploy | 🔧 | `infra/cloudformation/edge.yaml` (CloudFront+WAFv2+ACM) **✅ ships as a template**; `observability.yaml` **✅**; one-shot nesting of edge+regional **⬜** | Templates parse/lint (`docs/AWS-DEPLOYMENT-VALIDATION.md`); a single `make` target that nests edge (us-east-1) + regional stacks in one shot is outstanding. |
