...
3. **Masking before inference.** The student-PII masker runs at the prompt/audit boundary. Its always-on deterministic pass replaces **structured** identifiers — DOB-precision dates, addresses, phone/email, cards, and long numeric IDs — with stable pseudonyms **before** content enters a prompt or audit record (pii_masker `mask()` / `masked_pseudonym()`). Free-text personal **names** and other unstructured PII have no regex signature and are masked only by the ML NER pass (`MASK_ENGINE=ml`), which is **mandatory and fail-closed when processing real education-record text** (`ALLOW_REAL_DATA=1` → the masker raises rather than leak a name); the regex-only default is for demo/fixture and public reference data.
