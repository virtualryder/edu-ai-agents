...
- **Student-PII masker** (`platform_core/edu_agent_platform/pii_masker/`): a deterministic regex + Luhn pass (always on) replaces **structured** FERPA "Safe Harbor"-style identifiers — SSN, student/case IDs, email, phone, DOB-precision dates, addresses, card/lunch numbers — with stable pseudonyms before any inbound tool result enters a prompt or audit record. Free-text personal **names** and other unstructured PII have no regex signature and are masked only by the optional ML NER pass (`MASK_ENGINE=ml`, Comprehend/Presidio); that pass is **mandatory and fail-closed in real-data mode** (`ALLOW_REAL_DATA=1` → the masker raises `RealDataMaskingError` rather than leak a name via regex-only masking) and best-effort otherwise. Stable pseudonyms allow cross-call tracing without exposing FERPA-protected identifiers or COPPA-protected data for under-13 learners.
