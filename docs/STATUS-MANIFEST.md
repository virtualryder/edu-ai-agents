...
- **Unit = Yes (all):** the platform and governance suites pass (`platform_core/tests`, `governance/tests`, `infra/lambdas/agentcore_provisioner`) — 67 tests pass via `python -m pytest platform_core/tests governance/tests infra/lambdas/agentcore_provisioner -q --ignore=governance/tests/test_hitl_gates.py`. Each agent additionally ships a `tests/` suite.
