...
| **Data residency and sovereignty** — student records stay in institution's AWS account and region | Enables (region selection, no cross-region replication by default) | Owns (architecture design; no SI systems receive student PII) | Owns (selects AWS region; approves data-flow architecture; ensures region meets state data-residency law) | Inference runs on Amazon Bedrock, reached from the institution's AWS account over AWS PrivateLink (a regional AWS service, not in-VPC hosting); student PII is masked before inference and no student PII transits SI infrastructure |
...
> "Student records stay within your AWS account; Bedrock is a regional AWS service reached only over AWS PrivateLink (not in-VPC hosting), in your selected region, and student PII is masked before inference. No student PII transits SI infrastructure. AWS's shared responsibility model means AWS owns physical security, hypervisor, and managed-service infrastructure; you own the data and the IAM policies."
